...
import { ChevronLeft, ChevronRight, CheckCircle2, Circle, Coins, ArrowRight, Building2 } from "lucide-react";
...
function Mieteingaenge() {
  ...
  if (!objekteGeladen || !einheitenGeladen || !mieterGeladen || !mietzahlungenGeladen) {
    return <LadeSkeleton titel="Mieteingänge" text="Behalten Sie den Überblick, wer die Miete schon gezahlt hat." />;
  }

  const istBezahlt = (mieterId: string) => mietzahlungen.some((z) => z.mieterId === mieterId && z.monat === monat);
  ...
  const gruppen = objekte
    .map((o) => {
      ...
      const zeilen = mieter
        ...
        .map((m) => ({ mieter: m, einheit: einheiten.find((e) => e.id === m.einheitId), betrag: erwarteteMieteFuerMonat(m, monat) }))
        .filter((z) => z.betrag > 0);
  ...
  const eingegangen = alleZeilen.filter((z) => istBezahlt(z.mieter.id)).reduce((s, z) => s + z.betrag, 0);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Behalten Sie den Überblick, wer die Miete schon gezahlt hat — je Mieter und Objekt.</p>
      </div>

      {/* Monatsnavigation */}
      <div className="flex items-center justify-between rounded-xl border bg-card p-3">
        <button onClick={() => setMonat((m) => shiftMonat(m, -1))} className="grid h-9 w-9 place-items-center rounded-md hover:bg-accent" aria-label="Vorheriger Monat">
          <ChevronLeft className="h-5 w-5" />
        </button>
        <div className="text-center">
          <div className="font-medium">{monatLabel(monat)}</div>
          {!istAktuellerMonat && (
            <button onClick={() => setMonat(monatKey())} className="text-xs text-primary hover:underline">
              Zum aktuellen Monat
            </button>
          )}
        </div>
        <button onClick={() => setMonat((m) => shiftMonat(m, 1))} className="grid h-9 w-9 place-items-center rounded-md hover:bg-accent" aria-label="Nächster Monat">
      ...
      {gruppen.length === 0 ? (
        <div className="rounded-2xl border border-dashed p-10 text-center">
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            Für {monatLabel(monat)} sind keine Mieter mit Kaltmiete hinterlegt. Legen Sie einen Mieter bei einer Einheit an, um Mieteingänge zu verfolgen.
          </p>
        ...
        <>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-xl border bg-card p-4">
              ...
              <div className="mt-1 text-2xl font-semibold tabular-nums text-blue-600">{fmtEUR(eingegangen)}</div>
            </div>
            <div className="rounded-xl border bg-card p-4">
              <div className="text-xs text-muted-foreground">Offen ({anzahlOffen})</div>
              <div className={"mt-1 text-2xl font-semibold tabular-nums " + (offen > 0 ? "text-amber-600" : "text-muted-foreground")}>
                {fmtEUR(offen)}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {gruppen.map(({ objekt, zeilen }) => {
              ...
              const objEingegangen = zeilen.filter((z) => istBezahlt(z.mieter.id)).reduce((s, z) => s + z.betrag, 0);
              return (
          ...
          <p className="text-xs text-muted-foreground">Tipp: Auf eine Zeile klicken schaltet zwischen „bezahlt" und „offen" um.</p>
        </>
      )}
    </div>
  );
